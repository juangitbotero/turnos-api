...
- **Demo feature batch — multi-day, CV, availability, experiences, calendar (2026-08-04):**
  ...
  - **Job titles:** added `Lavador/a de loiça` and a new **`Serviços e Manutenção`** category (`Empregado/a de limpeza`, `Técnico/a de manutenção`, `Segurança/Vigilante`). **One deliberate rename:** `Rececionista de hotel` and `Rececionista de eventos` were merged into a single generic **`Rececionista`**, which now appears in *both* Hotelaria and Eventos (the same title in two categories, exactly like `Barista`; `ALL_SKILLS` dedupes it). Renames normally orphan stored `worker.skills`/`shift.subcategory` strings, so this one is covered by **`LEGACY_SKILL_ALIASES` + `normalizeSkill()/normalizeSkills()`** in shared — the same pattern as `LEGACY_PAYMENT_METHOD_LABELS`. Applied on write (`updateWorkerProfile`, `updateWorkerPartialFields`) and on read (`getProfile`), so old rows keep working and heal on the worker's next save. Skill *matching* was unaffected either way: it uses `LIKE '%skill%'`, and `Rececionista` is a prefix of both retired titles. Optional one-off cleanup: `UPDATE workers SET skills = REPLACE(REPLACE(skills,'Rececionista de hotel','Rececionista'),'Rececionista de eventos','Rececionista');`
