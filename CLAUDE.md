...
- **Still pending (ops/config):** €45 Stripe price + `STRIPE_SUBSCRIPTION_PRICE_ID` update in Railway; Connect webhook endpoint creation; attorney written sign-off on the Pay Link structure; pre-shift consequence-reminder push (policy states it; not yet scheduled in code)
